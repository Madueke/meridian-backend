 ...
 *
 * When Hermes is configured the scheduler is a bridge: it keeps one Hermes
 * cron job per active user alarm (the per-alarm cadence lives in Hermes) and
 * harvests new job outputs to queue push notifications. Without Hermes it
 * falls back to the deterministic in-process strategy matcher.
 */
function startAlarmScheduler(intervalMinutes = DEFAULT_CHECK_INTERVAL_MINUTES) {
  const hermesAlarms = require('./hermes-alarms');
  if (hermesAlarms.isConfigured()) {
    const timer = hermesAlarms.startHermesAlarmBridge();
    console.log('[alarms] Hermes job bridge started (harvest every 2 minutes)');
    return timer;
  }
