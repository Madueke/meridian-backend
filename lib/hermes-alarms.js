  ...
  for (const d of desired) {
    ...
    try {
      if (!job) {
        ...
        // The jobs API returns `schedule` as a parsed object
        // ({kind, minutes, display}); compare against its display string.
        const jobSchedule = job.schedule_display || (job.schedule && job.schedule.display) || String(job.schedule || '');
        const changed =
          jobSchedule !== d.schedule || job.prompt !== d.prompt;
        if (changed) {
