...
// Build the Neutral Pip system prompt for a specific chat turn. Appends the
// authenticated-user binding so Hermes passes the server-derived user
// reference to the MT5 account tools — identity comes from the app session
// token, never from what the user types.
function buildSystemPrompt(userId) {
  const safeUserId = sanitizeSessionKey(userId || 'anonymous');
  return `${NEUTRAL_PIP_SYSTEM}

Authenticated user:
- Your current user id is: ${safeUserId}
- ALWAYS pass user_ref=${safeUserId} to the MT5 account tools
  (get_account_state, check_connection_status, place_trade).
- Never invent, guess, or change this value — it is supplied by the app's
  authenticated session, and using any other value just reports no
  connected account.`;
}
