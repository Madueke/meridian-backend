...
//  4. Claude: chart summary, setup match, quoted backtest stats, reasoning,
//     trade proposal (place_trade tool) — or deterministic fallback
...
const { analyzeWithClaude, fallbackAnalysis } = require('./claude');
...
async function runAnalyze({ user_id, symbol, timeframe }) {
  ...
  // 4. Claude (or deterministic fallback).
  const llm = await analyzeWithClaude({
    symbol,
    timeframe,
    chart,
  ...
  const analysis = llm.ok
    ? {
        chart_summary: extractSummary(llm.text),
        strategy_match: null,
        backtest_accuracy: null,
        reasoning_text: llm.text,
        proposed_trade: llm.proposed_trade,
        llm: 'claude',
      }
    : fallbackAnalysis({
        symbol,
  ...
  if (!analysis.proposed_trade) {
    ...
    if (!autoExecute) {
      ...
      const result = await mt5Bridge.executeTrade(analysis.proposed_trade);
      actionTaken = {
...
/** First sentence(s) of the Claude text as a short chart summary. */
function extractSummary(text) {
  const first = String(text || '').split(/\n{2,}/)[0] || '';
  const lines = first.split('\n').filter(Boolean);
  return lines.slice(0, 2).join(' ') || first.slice(0, 200);
}
