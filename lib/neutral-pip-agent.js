...
async function runAgent(params) {
  ...
  // Legacy fallback only: persistent memory and context injection now live in
  // Hermes (scoped per user via X-Hermes-Session-Key on the primary chat
  // path). Do not inject the custom memory store here, or both systems would
  // add context to the same conversation.
  const systemPrompt = NEUTRAL_PIP_SYSTEM;
...
module.exports = { runAgent, NEUTRAL_PIP_SYSTEM, TOOLS };
