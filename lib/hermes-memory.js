...
function initDb() {
  ...
  // Migration for DBs created before the skills table gained source/active.
  // CREATE TABLE IF NOT EXISTS never alters an existing table, so add the
  // columns explicitly when they are missing.
  const skillColumns = db.prepare('PRAGMA table_info(skills)').all().map((c) => c.name);
  if (!skillColumns.includes('source')) {
    db.exec(`ALTER TABLE skills ADD COLUMN source TEXT DEFAULT 'auto'`);
    console.log('[hermes-memory] Added skills.source column');
  }
  if (!skillColumns.includes('active')) {
    db.exec('ALTER TABLE skills ADD COLUMN active INTEGER DEFAULT 1');
    console.log('[hermes-memory] Added skills.active column');
  }
...
function setSkillActive(sessionId, name, active) {
  if (!db) initDb();
  ...
  stmt.run(active ? 1 : 0, Date.now(), sessionId, name);
}

function getSkillsBySource(sessionId, source) {
  if (!db) initDb();
  
  const stmt = db.prepare(`
    SELECT id, name, description, content, source, active, use_count, created_at, updated_at
    FROM skills
    WHERE session_id = ? AND source = ?
    ORDER BY use_count DESC
  `);
  
  return stmt.all(sessionId, source);
}

function updateSkillSource(sessionId, name, source) {
  if (!db) initDb();
  ...
  stmt.run(source, Date.now(), sessionId, name);
}
